...
export class WeatherGatewayError extends Error {
  ...
  constructor(input: { code: string; message: string; city?: string; status?: number }) {
    super(input.message);
...
export function createWeatherGateway(options: WeatherGatewayOptions = {}) {
  return {
